...
## This is the QA automation project for `bank-app`

This directory now lives inside `bank-app` itself, at `automation/` — every
command in this README runs from there, and the app under test is this
directory's parent. This repo (`3P-CIBC-Automation`) remains the historical
record of the project's development up to that move, but is no longer where
the automation code, or its CI, lives.
